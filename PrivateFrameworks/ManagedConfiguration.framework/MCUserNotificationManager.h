/*
 *     Generated by class-dump 3.3.4 (64 bit).
 *
 *     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2011 by Steve Nygard.
 */

#import "NSObject.h"

@interface MCUserNotificationManager : NSObject
{
}

+ (id)sharedManager;
- (void)mainQueueDidReceiveAppWhitelistChangedNotification;
- (void)cancelAllNotificationsCompletionBlock:(id)arg1;
- (void)displayUserNotificationWithTitle:(id)arg1 message:(id)arg2 defaultButtonText:(id)arg3 alternateButtonText:(id)arg4 displayOnLockScreen:(_Bool)arg5 displayInAppWhitelistModes:(_Bool)arg6 dismissAfterTimeInterval:(double)arg7 assertion:(id)arg8 completionBlock:(id)arg9;
- (void)inviteUserToVPPWithTitle:(id)arg1 message:(id)arg2 assertion:(id)arg3 completionBlock:(id)arg4;
- (void)promptUserToLogIntoiTunesWithTitle:(id)arg1 message:(id)arg2 assertion:(id)arg3 completionBlock:(id)arg4;
@property(readonly, nonatomic) _Bool hasOutstandingNotifications;
- (void)dealloc;
- (id)init;

@end

